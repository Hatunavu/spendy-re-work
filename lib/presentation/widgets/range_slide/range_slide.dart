...
typedef RangeSliderCallback = Function(double lowerValue, double upperValue);
typedef RangeSliderValueIndicatorFormatter = Function(int index, double value);
...
class _RangeSliderState extends State<RangeSlider>
  ...
  static const Duration kEnableAnimationDuration = Duration(milliseconds: 75);
  ...
  double _unlerp(double value) {
    assert(value <= widget.max, 'values smaller or equal than max');
    assert(value >= widget.min, 'values bigger or equal than min');
  ...
  double lerp(double value) {
    assert(value >= 0.0, 'value bigger or equal than 0');
    assert(value <= 1.0, 'value smaller or equal than');
...
class _RenderRangeSlider extends RenderBox {
  _RenderRangeSlider({
    ...
    _touchRadiusExpansionRatio = touchRadiusExpansionRatio!;
  ...
  // -------------------------------------------------
  late _RangeSliderState state;
  RangeSliderCallback? _onChanged;
  late RangeSliderCallback _onChangeStart;
  late RangeSliderCallback _onChangeEnd;
  ...
  late double _touchRadiusExpansionRatio;
  ...
  late bool _allowThumbOverlap;
  late Size sizeWithOverflow;
  ...
  set lowerValue(double? value) {
    assert(
        value != null && value >= 0.0 && value <= 1.0, 'value range in 0..1');
  ...
  set touchRadiusExpansionRatio(double? value) {
    assert(value != null && value >= 0.1, 'value bigger than 0.1');
  ...
  void _paintTrack(Canvas canvas, Offset offset) {
    ...
    final Paint unselectedTrackPaint = Paint()
      ..color = (isInteractive
          ? _sliderTheme.inactiveTrackColor
          : _sliderTheme.disabledInactiveTrackColor)!;
    final Paint selectedTrackPaint = Paint()
      ..color = (isInteractive
  ...
  void _paintTickMarks(Canvas canvas, Offset offset) {
    ...
    final double _tickRadius =
        (_sliderTheme.trackHeight! / 2.0).clamp(1.0, 2.0);
  ...
  void _paintThumbs(PaintingContext context, Offset offset) {
    ...
    // Paint the thumbs, via the Theme, Check Text scale factor
      ...
      if (_valueIndicatorAnimation.status != AnimationStatus.dismissed &&
        ...
        _valueIndicatorPainter
          ..text = TextSpan(
            ...
            text: '${int.parse(textValue).formatMoneyWithChar()}',
          )
          ..layout();
  ...
  void _validateActiveThumb(Offset position) {
    final _thumbLowerExpandedRect = Rect.fromCircle(
        center: _thumbLowerRect.centerLeft,
        radius: _thumbRadius * _touchRadiusExpansionRatio);
    final _thumbUpperExpandedRect = Rect.fromCircle(
        center: _thumbUpperRect.centerRight,
        radius: _thumbRadius * _touchRadiusExpansionRatio);
    final double calculatedDivisionOffset = (_divisions != null)
        ? _discretize(1.0 / _divisions!)
        : (_thumbRadius * 2.0) / _trackLength;
    final double divisionOffset =
        _allowThumbOverlap ? 0.0 : calculatedDivisionOffset;
