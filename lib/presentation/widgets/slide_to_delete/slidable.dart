...
import 'package:spendy_re_work/presentation/widgets/slide_to_delete/fractionnally_aligned_sized_box.dart.dart';
import 'package:spendy_re_work/presentation/widgets/slide_to_delete/slidable_dismissal.dart.dart';
...
class SlideActionBuilderDelegate extends SlideActionDelegate {
  ...
  const SlideActionBuilderDelegate({
    ...
    required this.actionCount,
            ...
            //actionCount != null &&
            actionCount >= 0,
            'action count bigger or equal than 0');
...
class SlidableData extends InheritedWidget {
  ...
  static SlidableData? of(BuildContext context) {
    return context.dependOnInheritedWidgetOfExactType(aspect: SlidableData);
  }
...
class SlidableController {
  ...
  late SlidableState? _activeState;
...
class Slidable extends StatefulWidget {
  ...
  Slidable.builder(
      ...
      : assert(actionPane != null, ''),
        ...
        assert(closeOnScroll != null, 'close on scroll not null'),
        assert(enabled != null, 'enable not null'),
  ...
  static SlidableState? of(BuildContext context) {
    return context.dependOnInheritedWidgetOfExactType(aspect: _SlidableScope);
  }
...
class SlidableState extends State<Slidable>
  ...
  late Animation<double> _actionsMoveAnimation;
  ...
  void _startResizeAnimation() {
    assert(
        _overallMoveController != null, 'overall move controller is not null');
          ...
          if (_resizeAnimation != null) {
            ...
            assert(() {
              if (_resizeAnimation!.status != AnimationStatus.forward) {
                assert(
                    _resizeAnimation!.status == AnimationStatus.completed, '');
                throw FlutterError(
