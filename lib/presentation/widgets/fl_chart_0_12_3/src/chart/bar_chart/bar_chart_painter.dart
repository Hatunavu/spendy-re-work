...
import 'package:spendy_re_work/presentation/widgets/fl_chart_0_12_3/src/chart/bar_chart/bar_chart_data.dart';
...
class BarChartPainter extends AxisChartPainter<BarChartData>
  ...
  late List<_GroupBarsPosition> _groupBarsPosition;
  ...
  List<double> _calculateGroupsX(Size viewSize,
      ...
      for (var j = 0; j < barGroup.barRods.length; j++) {
        ...
        if (barRod.backDrawRodData.show && barRod.backDrawRodData.y != 0) {
          if (barRod.backDrawRodData.y > 0) {
            ...
            final bottom = max(getPixelY(barRod.backDrawRodData.y, drawSize),
                top + cornerHeight);
          ...
          if (barRod.backDrawRodData.colors.length == 1) {
            _barPaint
              ..color = barRod.backDrawRodData.colors[0]
              ..shader = null;
          } else {
            ...
            List<double>? stops;
            if (barRod.backDrawRodData.colorStops?.length !=
                barRod.backDrawRodData.colors.length) {
        ...
        if (barRod.y != 0) {
          if (barRod.y > 0) {
            ...
            final bottom = getPixelY(max(data.minY!, 0), drawSize);
            final top =
                min(getPixelY(barRod.y, drawSize), bottom - cornerHeight);
            ...
            final top = getPixelY(min(data.maxY!, 0), drawSize);
            final bottom =
                max(getPixelY(barRod.y, drawSize), top + cornerHeight);
  ...
  BarTouchResponse handleTouch(FlTouchInput touchInput, Size size) {
    final touchedSpot = _getNearestTouchedSpot(
        size, touchInput.getOffset(), _groupBarsPosition);
