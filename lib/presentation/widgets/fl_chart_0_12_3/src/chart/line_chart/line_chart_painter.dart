...
import 'package:spendy_re_work/presentation/widgets/fl_chart_0_12_3/src/chart/base/axis_chart/axis_chart_data.dart';
import 'package:spendy_re_work/presentation/widgets/fl_chart_0_12_3/src/chart/base/axis_chart/axis_chart_painter.dart';
...
import 'package:spendy_re_work/presentation/widgets/fl_chart_0_12_3/src/extensions/canvas_extension.dart';
import 'package:spendy_re_work/presentation/widgets/fl_chart_0_12_3/src/extensions/path_extension.dart';
import 'package:spendy_re_work/presentation/widgets/fl_chart_0_12_3/src/extensions/paint_extension.dart';
...
class LineChartPainter extends AxisChartPainter<LineChartData>
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var i = 0; i < data.showingTooltipIndicators!.length; i++) {
      ...
      _drawTouchTooltip(canvasWrapper, data.lineTouchData!.touchTooltipData,
  ...
  void _drawBarLine(CanvasWrapper canvasWrapper, LineChartBarData barData) {
    ...
    for (final bar in barList) {
      ...
      _drawBelowBar(
          canvasWrapper, belowBarPath, completelyFillAboveBarPath, barData);
      _drawAboveBar(
          canvasWrapper, aboveBarPath, completelyFillBelowBarPath, barData);
  ...
  void _drawBetweenBarsArea(CanvasWrapper canvasWrapper, LineChartData data,
    ...
    final spots = <FlSpot>[...toBarData.spots!.reversed.toList()];
  ...
  void _drawTouchedSpotsIndicator(
      CanvasWrapper canvasWrapper, LineChartBarData barData) {
    final viewSize = canvasWrapper.size;
    ...
    for (var i = 0; i < barData.showingIndicators.length; i++) {
      ...
      /// For drawing the dot
      final showingDots =
          //indicatorData.touchedSpotDotData != null &&
      ...
      final lineEnd = data.lineTouchData!.fullHeightTouchLine
          ? top
      ...
      canvasWrapper.drawDashedLine(bottom, lineEnd, _touchLinePaint,
  ...
  void _drawBelowBar(CanvasWrapper canvasWrapper, Path belowBarPath,
    ...
    if (barData.belowBarData.colors.length == 1) {
      _barAreaPaint
        ..color = barData.belowBarData.colors[0]
        ..shader = null;
    } else {
      List<double>? stops;
  ...
  void _drawAboveBar(CanvasWrapper canvasWrapper, Path aboveBarPath,
    ...
    if (barData.aboveBarData.colors.length == 1) {
      _barAreaPaint
        ..color = barData.aboveBarData.colors[0]
        ..shader = null;
    } else {
      List<double>? stops;
  ...
  void _drawBetweenBar(CanvasWrapper canvasWrapper, Path aboveBarPath,
    ...
    if (betweenBarsData.colors.length == 1) {
      _barAreaPaint
        ..color = betweenBarsData.colors[0]
        ..shader = null;
    } else {
      var stops = <double>[];
    ...
    canvasWrapper
      ..saveLayer(Rect.fromLTWH(0, 0, viewSize.width, viewSize.height), Paint())
      ...
      while (verticalSeek! <= data.maxY!) {
        if (leftTitles.checkToShowTitle(
          ...
          final tp = TextPainter(
            ...
            ..layout(maxWidth: getExtraNeededHorizontalSpace());
          x -= tp.width + leftTitles.margin;
          y -= tp.height / 2;
          canvasWrapper
            ..save()
            ..translate(x + tp.width / 2, y + tp.height / 2)
            ..rotate(radians(leftTitles.rotateAngle))
            ..translate(-(x + tp.width / 2), -(y + tp.height / 2));
          y -= translateRotatedPosition(tp.width, leftTitles.rotateAngle);
          canvasWrapper
            ..drawText(tp, Offset(x, y))
            ..restore();
        }
      ...
      while (horizontalSeek! <= data.maxX!) {
        if (topTitles.checkToShowTitle(
          ...
          canvasWrapper
            ..save()
            ..translate(x + tp.width / 2, y + tp.height / 2)
            ..rotate(radians(topTitles.rotateAngle))
            ..translate(-(x + tp.width / 2), -(y + tp.height / 2));
      ...
      while (verticalSeek! <= data.maxY!) {
        if (rightTitles.checkToShowTitle(
          ...
          canvasWrapper
            ..save()
            ..translate(x + tp.width / 2, y + tp.height / 2)
            ..rotate(radians(rightTitles.rotateAngle))
            ..translate(-(x + tp.width / 2), -(y + tp.height / 2));
          y += translateRotatedPosition(tp.width, leftTitles.rotateAngle);
          canvasWrapper
            ..drawText(tp, Offset(x, y))
      ...
      while (horizontalSeek! <= data.maxX!) {
        if (bottomTitles.checkToShowTitle(data.minX, data.maxX, bottomTitles,
          ...
          canvasWrapper
            ..save()
            ..translate(x + tp.width / 2, y + tp.height / 2)
            ..rotate(radians(bottomTitles.rotateAngle))
            ..translate(-(x + tp.width / 2), -(y + tp.height / 2));
    ...
    if (data.extraLinesData!.verticalLines.isNotEmpty) {
      for (final line in data.extraLinesData!.verticalLines) {
        ...
        final bottomChartPadding =
            getExtraNeededVerticalSpace() - getTopOffsetDrawSize();
        final to = Offset(getPixelX(line.x!, chartUsableSize),
