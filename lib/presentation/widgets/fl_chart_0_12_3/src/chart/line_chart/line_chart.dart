    ...
    return MouseRegion(
      onEnter: (e) {
        ...
        final LineTouchResponse response = _touchHandler?.handleTouch(
            FlPanStart(e.localPosition), chartSize) as LineTouchResponse;
      ...
      onExit: (e) {
        ...
        final LineTouchResponse response = _touchHandler?.handleTouch(
                FlPanEnd(Offset.zero, Velocity.zero), chartSize)
            as LineTouchResponse;
        if (_canHandleTouch(response, touchData!)) {
          touchData.touchCallback!(response);
        }
      },
      onHover: (e) {
        ...
        final LineTouchResponse response = _touchHandler?.handleTouch(
            FlPanMoveUpdate(e.localPosition), chartSize) as LineTouchResponse;
        if (_canHandleTouch(response, touchData!)) {
          touchData.touchCallback!(response);
        }
      },
      child: GestureDetector(
        ...
        onLongPressEnd: (d) {
          ...
          final LineTouchResponse response = _touchHandler?.handleTouch(
              FlLongPressEnd(d.localPosition), chartSize) as LineTouchResponse;
          if (_canHandleTouch(response, touchData!)) {
            touchData.touchCallback!(response);
          }
        },
        ...
        onPanCancel: () {
          ...
          final LineTouchResponse response = _touchHandler?.handleTouch(
              ...
              chartSize) as LineTouchResponse;
          if (_canHandleTouch(response, touchData!)) {
            touchData.touchCallback!(response);
          }
        },
    ...
    if (touchResponse.touchInput is FlPanStart ||
      ...
      setState(() {
        ...
        _showingTouchedTooltips
          ..clear()
          ..add(ShowingTooltipIndicators(0, sortedLineSpots));
      });
    } else {
      setState(() {
