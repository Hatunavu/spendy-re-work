...
import 'package:spendy_re_work/presentation/widgets/rate_my_app/conditions.dart';
...
class RateMyApp {
  ...
  RateMyApp({
    ...
    this.appStoreIdentifier,
        ...
        assert(preferencesPrefix != null, 'preferencesPrefix not null') {
  ...
  //assert(conditions != null,'conditions not null');
  ...
  Future<void> showRateDialog(
    ...
    final bool? isNativeReviewDialog = await isNativeReviewDialogSupported;
    if (!ignoreNativeDialog && isNativeReviewDialog!) {
  ...
  Future<LaunchStoreResult> launchStore() async {
    final int result = await _channel.invokeMethod('launchStore',
  ...
  void populateWithDefaultConditions({
    ...
    conditions
      ..add(MinimumDaysCondition(
        minDays: minDays ?? 7,
        remindDays: remindDays ?? 7,
      ))
      ..add(MinimumAppLaunchesCondition(
        minLaunches: minLaunches ?? 10,
        remindLaunches: remindLaunches ?? 10,
      ))
      ..add(DoNotOpenAgainCondition());
  }
}
