...
class Age {
  ...
  static double dateDifferenceDoubleMonth(
      {required DateTime fromDate,
      required DateTime toDate,
      bool includeToDate = false}) {
    final age = dateDifference(
        fromDate: fromDate, toDate: toDate, includeToDate: includeToDate);
    double resultMonth = age.years * 12.0 +
        age.months.toDouble() +
        double.parse((age.days / 30).toStringAsFixed(2));
    if (resultMonth < 1.0) {
      resultMonth = 1.0;
    }
    return resultMonth;
